function [x, w, v, ders] = lagpts_new(n, int, meth, alf)
...
%   April 2009 - GLR [2] added for N >= 1 29.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ADAPTED by Emma Nel, Nick Hale, 2026                                         % 
%  - GLR routine                                                               %
%    Compute barycentric weights and output ders                               %
%  - GLR routine -> eval_Lag                                                   %
%    Standard recurrence relation replaced by modified recurrence relation     %
%  - GLR routine -> alg1_Lag                                                   %
%    Increased # terms in Taylor series for small n and final roots            %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     [x, w] = newton(n, Wmethod, alpha, 0);
    [x, w, v, ders] = lag_rec(n, alpha, Wmethod);
...
v = v./max(abs(v));            % Normalise so that norm(v, inf) = 1
...
function [x, w, v, ders] = lag_rec(n, alpha, flag)
...
for i = 1:n
    ...
    pp = (n*p1 - (n+alpha)*p2)/z/realmin; % derivative
    ders(i) = pp;
...
if n < 30 
    % increase for better accuracy for small n
    m = 60;
else 
    m = 30;
end
...
for j = n1:(n - 1)
    ...
    % increase # terms in Taylor series for better accuracy at final roots
    if ( j == (n-5) ), m = 60; hh1 = ones(m+1, 1); zz = zeros(m, 1); end
...
% S. Huang and H. Yu, "Improved Laguerre spectral methods with less
% round-off errors and better stability", Journal of Scientific Computing,
% 101:72, 2024.
Lm1 = (1-x).*exp(-x/2); 
dLm1 = (-x).*exp(-x/2); 
Lpm1 = -0.5.*Lm1 - exp(-x/2).*ones(size(x));
for k = 1:n-1
    dL = (k*dLm1 - x*Lm1) / (k+1);
    % update function
    L = Lm1 + dL;
    % update der
    Lp = Lpm1 - 0.5*Lm1 - 0.5*L;
    % save
    Lm1 = L; Lpm1 = Lp; dLm1 = dL;
end
